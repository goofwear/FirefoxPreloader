...
protected:
	...
	afx_msg void OnContextmenuUnloadfirefoxpreloader();
	afx_msg void OnContextmenuInternetoptions();
	afx_msg void OnContextmenuAboutfirefoxpreloader();
	afx_msg void OnContextmenuUnloadfirefox();
	afx_msg void OnContextmenuReloadfirefox();
...
// Specialized internal helper functions
	BOOL IsFirefoxRunning();						// Determins if an instance of Firefox is already running
	BOOL PreloadFF();								// Pre-loads a hidden instance of firefox into memory
	BOOL UnloadFF();								// Unloads the instance of firefox started by the Preloader
...
// Internal program status
	...
	CString p_szProfilePath;						// The path to the current Firefox profile
	BOOL p_bUserLock;								// We will also use the lock file to determine if Firefox is running

// Presistant program settings
	UINT p_nTimerInt;								// Interval to poll for Firefox
	BOOL p_bAutoFindFF;								// Automaticly discover the Firefox path
	BOOL p_bWarnOnUnloads;							// Warns the user about any action that will unload Firefox
	CString p_szFirefoxPath;						// The path to the Firefox directory
