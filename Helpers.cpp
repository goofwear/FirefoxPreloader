...
/////////////////////////////////////////////////////////////////////////////
// Finds the path to Firefox in the registry using the key for the latest version
/////////////////////////////////////////////////////////////////////////////
// pszFirefoxPathBuffer can be NULL to simply test if we can even find
// a valid path to Firefox.
...
BOOL GetCurrentFirefoxProfile(CString *pszProfilePathBuffer) {
	...
	// Trim the \ off if it is there and add the Mozilla\Firefox
	if (szADataPath[szADataPath.GetLength() - 1] == _T('\\')) szADataPath.TrimRight(_T('\\'));
	szADataPath += "\\Mozilla\\Firefox";
	...
	::OutputDebugString(_T("Found the Mozilla Firefox profiles.ini\r\n"));
	CloseHandle(hUseful);
...
BOOL GetLatestFirefoxInstallDir(CString *pszFirefoxPathBuffer) {
	...
	CRegistryKey *moziKey = cReg.KeyLocalMachine()->OpenKey(_T("SOFTWARE\\Mozilla\\Mozilla Firefox"), KEY_READ);
	...
	::OutputDebugString(_T("Found Mozilla Firefox's root key\r\n"));
		...
		if (mozvKey != NULL) {
			...
			if (fCurrentVer > fLastVer) {
				...
				if (mozvKey->GetStringValue(_T("Install Directory"), &szCurrentPath) == ERROR_SUCCESS) {
					...
					// Make the firefox.exe path
					szFFExe = szCurrentPath;
					szFFExe += _T("\\firefox.exe");

					// See if firefox.exe exists in the path by opening it, only then do we have the right key
	...
	if (pszFirefoxPathBuffer != NULL) *pszFirefoxPathBuffer = szPath;
...
// Load and Wait for Firefox working thread function
UINT LoadAndWaitForFirefox(LPVOID pParam) {
	CString										// String Variables
		szFF,									// Firefox.exe path and command-line
	...
	if (!mutex.Lock(1000)) {
		::OutputDebugString(_T("Unable to lock loading Mutex, firefox may still be loaded\r\n"));
		return FALSE;
	}

	// Create the command-line to start Firefox
	szFF = pDlg->p_szFirefoxPath;
	szFF += _T("\\firefox.exe -chrome \"chrome://browser/content/web-panels.xul\"");
	...
	// Load firefox, hidden
	if (!::CreateProcess(NULL, szFF.GetBuffer(szFF.GetLength()), NULL, NULL, TRUE, NORMAL_PRIORITY_CLASS, NULL, pDlg->p_szFirefoxPath, &si, &pi)) {
		::OutputDebugString(_T("Firefox failed to run...\r\n"));
